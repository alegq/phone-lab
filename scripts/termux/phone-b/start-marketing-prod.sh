...
# Start prod api-marketing (NestJS dist/main.js).
set -euo pipefail
...
LOG_FILE="$LOG_DIR/marketing-prod.log"
...
echo "$(date -Iseconds) start: api-marketing-prod" >> "$LOG_FILE"
nohup "$NODE" dist/main.js >> "$LOG_FILE" 2>&1 &
echo "api-marketing-prod: started (port from .env, default 4008)"
