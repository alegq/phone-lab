...
echo "$(date -Iseconds) start: api-auth-prod" >> "$LOG_FILE"
nohup "$NODE" dist/main.js >> "$LOG_FILE" 2>&1 &
echo "api-auth-prod: started (port from .env, default 4001)"
