...
# Install watchdog cron + crond boot hook on phone-a.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
WATCH_SCRIPT="$HOME/phone-lab/scripts/termux/phone-a/watch-stack-phone-a.sh"
...
mkdir -p "$HOME/phone-lab/scripts/termux/lib" "$HOME/phone-lab/scripts/termux/phone-a"

if [ ! -f "$WATCH_SCRIPT" ]; then
  if [ -f "$SCRIPT_DIR/watch-stack-phone-a.sh" ]; then
    cp "$SCRIPT_DIR/watch-stack-phone-a.sh" "$WATCH_SCRIPT"
...
TMP_CRON="$(mktemp)"
crontab -l 2>/dev/null | grep -v 'watch-stack-phone-a.sh' >"$TMP_CRON" || true
...
echo "Installed watchdog cron on phone-a:"
